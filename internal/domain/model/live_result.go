...
type LiveElectionResult struct {
	...
	VoteShareInRegion              float64   `db:"vote_share_in_region"`
	RegionalDistributionPercentage float64   `db:"regional_distribution_percentage"`
...
type ElectionSummary struct {
	...
	TotalVoteAttempts          uint64    `db:"total_vote_attempts"`
	OverallVoteSharePercentage float64   `db:"overall_vote_share_percentage"`
	LastUpdated                time.Time `db:"last_updated"`
}
...
type CityRanking struct {
	...
	VoteDistributionPercentage float64   `db:"vote_distribution_percentage"`
	LastUpdated                time.Time `db:"last_updated"`
}
